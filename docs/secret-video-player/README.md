...
| 文档 | 内容 | 适合读者 |
| --- | --- | --- |
| [UX1 交互与功能优化方案 v1.0（2026-09-15）](design/2026-09-15-ux-functional-optimization-v1.0.md) | 播放与续播、媒体库、批量任务及布局优化；初始方案；实施进度见 [UX1 实施记录](reference/UX1-IMPLEMENTATION-AND-VALIDATION.md)，历史版本见 [设计索引](design/README.md) | 产品、开发者、测试人员 |
...
5. 手动上一项/下一项按当前筛选列表顺序导航并恢复历史；连续播放默认关闭，只在自然播放结束后前进一项，到列表末尾停止且不循环。
...
- “开始执行”旁显示当前缺少的条件；批次和逐项问题均可展开查看。
- “重试选中 / 重试全部”重新检查失败或取消项；成功项不重复执行。存在警告或输出路径变化时，先展示新计划，再由“开始执行”继续。
- 主动重新检查会废弃旧计划，取消或检查失败后需要再次检查。
- “清除完成记录”仅移除列表记录，保留输出文件。
- 编辑公开标题与描述时保持播放；取消丢弃草稿。保存时释放文件占用，随后恢复原位置及播放意图；失败保留草稿，关闭或换片后不恢复旧视频。
- 详细设计、开发检查与限制见 [UX1 实施与验证记录](reference/UX1-IMPLEMENTATION-AND-VALIDATION.md)。

### UX1 文件导入、输出和结果

- 加解密均可添加或拖入文件、目录；“包括子目录”默认关闭。导入可取消，格式不支持、重复或不可读取的输入会显示汇总。
- 加密器展开“批量输出与描述”，选择统一输出目录；可平铺或保留导入目录结构，应用到所选或全部未完成项。后续新导入文件沿用当前统一输出配置。
- Ctrl / Shift 多选用于批量输出与公开描述；“移除当前项 / 重试当前项”仍明确针对当前项。完成项只读，批量配置跳过完成项。
- “仅失败 / 取消”帮助集中处理问题；“定位文件”“复制路径”和“打开输出目录”用于查看完成结果。
- 复制加密结果路径后，在单文件播放器点击“粘贴路径”，或把一个 `.secvid` 文件拖入来源区，再输入该播放器的密码播放。当前 SDK 尚不支持从插件直接携带路径新建播放器标签。
