...
| 文档 | 内容 | 适合读者 |
| --- | --- | --- |
| [UX1 交互与功能优化方案 v1.1（2026-09-15）](design/2026-09-15-ux-functional-optimization-v1.1.md) | 实施后的交互规则、原生恢复与 SDK 能力边界；实施进度见 [UX1 实施记录](reference/UX1-IMPLEMENTATION-AND-VALIDATION.md)，历史版本见 [设计索引](design/README.md) | 产品、开发者、测试人员 |
...
1. 在宿主中打开“视频文件加密器”，添加或拖入视频、目录；“包括子目录”默认关闭。导入支持取消、去重和问题汇总。
2. 默认输出在源文件旁。展开“批量输出与描述”可选统一目录、保留导入目录结构；用 Ctrl / Shift 多选后应用输出或公开描述，仍可逐项编辑标题。完成项只读，批量配置会跳过。
3. 选择严格阻止或安全数字后缀策略，输入并确认至少 6 个字符的本批次密码。
4. 点击“检查批次”，确认可执行、冲突、警告和阻止数量。
5. 点击“开始执行”。任务严格顺序运行；可取消当前后继续，也可取消全部。

批次检查会验证输入、同路径、公开信息、批次内/磁盘重名、目录写入和按卷累计空间。编辑输出或公开信息会使旧计划失效，必须重新检查。“开始执行”旁说明缺失条件。“重试当前项 / 重试全部”只重查失败或取消目标；警告或输出变化时停留在新计划。主动重新检查会废弃旧计划，取消或失败后需要再次检查。
...
1. 在宿主中打开“加密视频播放器”。
2. 浏览选择、拖入一个 `.secvid` 文件，或点击“粘贴路径”接收加密结果路径。播放器先显示公开标题和描述，密码在本 Document 独立输入。
3. 输入密码后点击“播放”或“继续播放”。“从头播放”和“仅加载”是独立次级操作；加载期间可以取消。加载阶段执行 PBKDF2、固定头认证和 15 秒受限 LibVLC 回调媒体解析；LibVLC 3.0.23 返回干净 `Skipped` 时由后续真实播放、轨道和 Seek 门禁继续判定，不会完整解密视频。
4. 常驻播放、暂停、停止、进度、音量及六档倍速；“静音”可恢复原音量，“跳转”支持秒数、分:秒、时:分:秒。音轨、字幕按需展开；全屏覆盖宿主内容区。
5. 播放器获得焦点后可用 `Space`、左右方向键和上下方向键控制播放、5 秒 Seek 与 5% 音量；`M` 静音或恢复音量，全屏时按 `Esc` 退出。文本输入、下拉框、滑块、列表和按钮焦点不会被这些快捷键抢占。
...
2. 目录选择、排序和筛选直接显示；“库设置”默认折叠，其中可启用递归、刷新目录和管理历史。播放密码在需要时直接输入，也可从“更多”打开。
3. 搜索框始终可见，匹配磁盘文件名、公开标题和公开描述；清除搜索只重建当前列表投影，不会重新扫描磁盘。列表占据侧栏的主要高度，目录新增、删除、修改和改名仍会自动合并刷新。
4. 点击主按钮、双击列表项或按 `Enter` 都会恢复有效历史并立即播放。缺少密码时确认输入后继续原请求；取消会丢弃该请求。“更多”提供从头播放和仅加载。历史“未看完”与实时“正在播放／已暂停”分别展示，已知历史时长时显示观看进度。
5. “最近目录”最多保存 10 项，可快速打开或移除记录；离线目录会提示不可访问。“继续观看”按当前目录的未完成历史排序；“定位当前”展开列表并清除搜索及状态筛选。库设置可调侧栏宽度。
6. 手动上一项/下一项按当前筛选列表顺序导航并恢复历史；连续播放默认关闭，只在自然播放结束后前进一项，到列表末尾停止且不循环。

公开信息损坏的文件仍会以磁盘文件名显示并标注错误。此类文件仍可尝试播放，因为公开区损坏不必然意味着受认证保护的视频主体损坏。

最近目录列表、递归、排序、筛选、侧栏宽度与开关、音量、倍速和最多 1,000 条播放历史保存在当前用户
LocalAppData。路径与位置是明文隐私数据；密码、密钥、公开描述和轨道不落盘。侧栏提供清除
单项和二次确认清空全部历史。

### 批量解密视频

1. 在宿主中打开“批量视频解密器”，添加或拖入 `.secvid` 文件、目录；递归规则与加密器一致。
2. 选择统一输出目录并输入这些文件共用的密码。
3. 检查批次并查看问题，再开始执行。任务严格顺序执行；失败或取消项可在修正密码后重试，已经成功的项目会被跳过。
...
加解密均可用“仅失败 / 取消”集中处理问题；完成项支持“定位文件”“复制路径”，统一输出时可“打开输出目录”。“清除完成记录”只移除列表记录，不删除文件。加密结果复制路径后，在目标播放器粘贴路径或拖入文件即可进入观看流程。

### 编辑公开信息

标题和描述位于固定的 64 KiB 公开区，可以在不移动或重新加密视频主体的情况下原地修改。编辑时建立草稿并保持当前观看，取消丢弃草稿。保存时才释放文件占用，写入后使用本会话凭据恢复位置、播放/暂停、倍速和仍存在的轨道；失败保留草稿并说明恢复情况，关闭或换片后不恢复旧目标。
...
- 批量加密和解密不持久化队列或密码；关闭 Document 会取消当前工作并放弃等待项，已完成输出保留。
- 当前 SDK 暂无插件携带路径新建 Document 的公开接口；结果通过复制路径、粘贴或拖放交接。一键新建播放器标签待公开接口支持。
...
本轮设计、分批提交、本地单元/界面/原生验证及未覆盖场景见 [UX1 实施与验证记录](reference/UX1-IMPLEMENTATION-AND-VALIDATION.md)。
